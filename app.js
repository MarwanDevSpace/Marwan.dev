...
function render(route = "home", push = false) {
  ...
  if (push) {
    const path = route === "home" ? "/Marwan.dev" : `/${route.charAt(0).toUpperCase()}${route.slice(1)}`;
...
function getRouteFromPath() {
  ...
  let path = p ? p : url.pathname;

  if (path === "/" || path === "/Marwan.dev" || path === "/Marwan.dev/") return "home";
...
const PROMPTS_DATA = [
  {
    ...
    description: "امر لإنشاء مساعد شرعي يقوم بالبحث والتحليل الفقهي والأصولي بأسلوب عربي فصيح وبلاغة علمية",
  ...
  },
  {
    ...
    description: "امر لإنشاء مساعد تقني متقدم بإسلوب التتابع والبحث",
  ...
  },
  {
    id: "3",
    title: "MX1-GPT[AR]",
    description: "امر لتفعيل مودل عربي متقدم مبني على معيار البلاغة والفهم الدقيق",
